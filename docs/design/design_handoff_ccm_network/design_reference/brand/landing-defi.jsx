...
// Supports light + dark mode — both pulled from BI palettes (V1 Carbon · Forest).

const getDefiLight = () => window.biTokens;
const getDefiDark  = () => window.__BI_PALETTES['forest-dark'];
...
const CodePanel = ({ T, S, isDark }) => (
  <section style={{ padding: '64px 48px',
    ...
    <div style={{
      ...
      <div>
        <span style={{ color: '#c87a4a' }}>function</span>{' '}
        <span style={{ color: '#a8c89a' }}>wrap</span>(
        <span style={{ color: T.paper }}>uint256 tokenId, uint256 amount</span>
        ) <span style={{ color: '#c87a4a' }}>external</span> {'{'}<br/>
        {'  '}ccmNFT.<span style={{ color: '#a8c89a' }}>safeTransferFrom</span>(<br/>
        {'    '}msg.sender, <span style={{ color: '#c87a4a' }}>address</span>(vault),<br/>
        {'    '}tokenId, amount, <span style={{ color: '#e6b89a' }}>""</span><br/>
        {'  '});<br/>
        {'  '}ccmToken.<span style={{ color: '#a8c89a' }}>mint</span>(<br/>
        {'    '}msg.sender, amount * <span style={{ color: '#c87a4a' }}>1e18</span><br/>
        {'  '}); <span style={{ color: '#7a8a72' }}>{'// 1 NFT = 1 $CCM'}</span><br/>
        {'  '}<span style={{ color: '#c87a4a' }}>emit</span> Wrapped(msg.sender, tokenId, amount);<br/>
        {'}'}
      </div>
    </div>
  </section>
...
const LandingDefi = () => {
  ...
  const T = dark ? getDefiDark() : getDefiLight();
